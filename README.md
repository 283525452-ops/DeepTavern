...
*   双击 **`DeepTavern.vbs`** 启动前端监控台（无命令行）
#### 2. 开始新游戏 (New Game)
1.  在 **监控台** -> **存档管理** 页面。
2.  输入玩家名字，点击 **“新建存档”**。
...
*   ✅ **修改**：您可以修改源代码。
*   ✅ **分发**：您可以复制并分发本项目。
*   ✅ **私有化**：您可以修改后闭源使用（但必须保留原作者版权声明）。

**免责声明**：
DeepTavern 是一个逻辑编排框架。本项目**不包含**任何 LLM 模型权重文件。您在使用过程中下载或调用的第三方模型（如 Llama-3, Qwen, DeepSeek 等）需遵循其各自的许可协议（Model License）。
